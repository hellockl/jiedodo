...
class CI_Verify{
	...
	function GIFEncoder($GIF_src,$GIF_dly=100,$GIF_lop=0,$GIF_dis=0, $GIF_red=0,$GIF_grn=0,$GIF_blu=0,$GIF_mod='bin'){
		...
		CI_Verify::GIFAddHeader();
		for($i=0,$count_buf=count($this->BUF);$i<$count_buf;$i++){
			CI_Verify::GIFAddFrames($i,$GIF_dly[$i]);
		}
		CI_Verify::GIFAddFooter();
	}
	function GIFAddHeader(){
		...
		if(ord($this->BUF[0]{10})&0x80){
			...
			$this->GIF.="!\377\13NETSCAPE2.0\3\1".CI_Verify::GIFWord($this->LOP)."\0";
		}
	}
	function GIFAddFrames($i,$d){
		...
		if(ord($this->BUF[$i]{10})&0x80&&$this->IMG>-1){
			if($Global_len==$Locals_len){
				if(CI_Verify::GIFBlockCompare($Global_rgb,$Locals_rgb,$Global_len)){
...
//		return $imagedata;
		...
		return  $this->GetAnimation($imagedata);
	}
}
?>