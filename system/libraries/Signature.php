...
class CI_Signature{
	...
	public function __construct($ishttps=false)
    {
        $this->ci = &get_instance();
        if(!isset($this->ci->config->config['qingbo']['appkey'])|| empty($this->ci->config->config['qingbo']['appkey'])){
            die('appkey未配置');
        }

        if(!isset($this->ci->config->config['qingbo']['appsecret'])|| empty($this->ci->config->config['qingbo']['appsecret'])){
            die('appsecret未配置');
        }

        $app_key = $this->ci->config->config['qingbo']['appkey'];
        $app_secret = $this->ci->config->config['qingbo']['appsecret'];
	...
	public function curlQbPost($url, $header, $data)
    {
        $new_header= array();
